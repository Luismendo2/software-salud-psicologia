...
- [x] Generar e incluir iconos para web y móviles (favicons, apple-touch-icons, maskables).
- [x] Configurar el archivo de manifiesto (nombre, short_name, colores base).
- [x] Configurar el Service Worker para pre-cachear los assets principales de UI y cachear la ruta `/api/v1/patients/me/appointments`.
- [x] Crear y desplegar el componente `PwaInstallBanner` en el dashboard del paciente para sugerir instalación en iOS/Android.
...
- [x] Construir la vista `LibraryPage` en el panel del psicólogo.
- [x] Implementar la barra lateral de filtros (categorías de terapia, tipo de documento) y la barra de búsqueda en React.
...
- [x] Construir el modal en el portal del paciente y el botón de alerta persistente en el header.
- [ ] Implementar endpoint `POST /api/v1/crisis/alert` que envíe mensaje inmediato (SMS por Twilio de preferencia) al terapeuta.
- [x] Validar explícitamente en UI el descargo de responsabilidad sobre servicios de emergencia médica/nacional.
- [x] Implementar panel de configuración del protocolo de crisis para el psicólogo en historia clínica (`CrisisConfigTab`).
...
- [x] Añadir un badge visual (ej. ícono de exclamación o color ámbar) a la vista de calendario del psicólogo para citas de "alto riesgo".
...
- [x] Crear la página frontend pública tipo asistente (wizard): "¿Qué te gustaría tratar?", "¿Cuándo tienes tiempo?", etc.
- [x] Renderizar resultados sugeridos y conectar el flujo directamente a `BookingPublicPage` de la Feature 001.
